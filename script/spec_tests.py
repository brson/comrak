...
# This is adapted from the upstream CommonMark spec tester to test snoomark
# via its python bindings.
#
# Unfortunately this is a python 3 script, and we generate python 2 bindings,
# so this script needs to call out to a python 2 process instead of embedding
# cm_to_rtjson.py as a module.
...
def do_test(converter, test, normalize, result_counts, rtjson):
    ...
    if retcode == 0:
        ...
        if not rtjson:
            ...
            actual_json = json.loads(actual_html)
            try:
                expected_json = json.loads(expected_html)
            except:
                print(expected_html)
            actual_html = json.dumps(actual_json, indent=4, sort_keys=True)
            expected_html = json.dumps(expected_json, indent=4, sort_keys=True)
            passed = actual_html == expected_html
        if passed:
